...
|---|---|---|
| **site-director** | дирижёр · точка входа | процесс от концепта до деплоя, спека `DESIGN.md`, вызов остальных, режим rebuild |
...
```
          ...
          ┌──────────── site-director ────────────┐
          │  1. точка зрения                        │
          │  2. DESIGN.md (спека до кода) ──────────┼─► утверждение словами
...
Единый источник правды — файл `DESIGN.md`, который `site-director` пишет в корень проекта **до вёрстки**. Он переживает сжатие контекста и правится словами. Референс-библиотека (13 модулей) подгружается только когда доходит до нужного шага, а не висит в контексте.
...
Библиотека сайто-ориентирована и не дублирует вкус-инструмент: это фолбэк и опинионейтед-крафт, а не второй каталог палитр.
